...
//     = Future<({int newPage, bool hasMore, List<GoodDeedBlock> blocks})>;

// /// Represents an update to a page.
// typedef ListPostMapper = List<GoodDeedBlock> Function(List<Post> post);
...
//   /// Converts a list of [Post] objects to a list of [GoodDeedBlock] objects
...
//   /// For each filtered post, it creates an [GoodDeedBlock] object representing
//   /// the reel block and adds it to the `instaBlocks` list.
//   /// Finally, it returns the list of [GoodDeedBlock] objects.
//   ///
//   /// Example usage:
//   /// ```dart
//   /// List<Post> posts = [...];
...
//   /// ```
//   List<GoodDeedBlock> postsToReelBlockMapper(List<Post> posts) {
//     final instaBlocks = <GoodDeedBlock>[];
...
//   /// ```
//   List<GoodDeedBlock> postsToLargeBlocksMapper(List<Post> posts) =>
//       posts.map<GoodDeedBlock>((post) => post.toPostLargeBlock).toList();

//   /// Inserts sponsored blocks into a list of [GoodDeedBlock].
//   ///
...
//   /// [ReceivePort].
//   /// 5. Returns the inserted blocks as a list of [GoodDeedBlock].
//   ///
//   /// Example usage:
//   /// ```dart
//   /// final hasMore = true;
...
//   Future<List<GoodDeedBlock>> insertSponsoredBlocks({
//     required bool hasMore,
//     required List<GoodDeedBlock> blocks,
//   }) async {
...
//     final insertedBlocks = await receivePort.first as List<GoodDeedBlock>;
...
//     final blocks = args[2] as List<GoodDeedBlock>;
...
//     final sponsored =
//         sponsoredBlocksListJson.take(20).map(GoodDeedBlock.fromJson).toList();
...
// extension on List<GoodDeedBlock> {
//   List<PostBlock> selectPostsBlock() => whereType<PostBlock>().toList();

//   GoodDeedBlock? findPostBlock({
//     PostBlock? other,
...
mixin FeedBlocMixin on Bloc<FeedEvent, FeedState> {
  ...
  List<GoodDeedBlock> postsToLargeBlocksMapper(List<Post> posts) => posts
      .map<GoodDeedBlock>(
          (post) => PostToLargeBlockConverter(post).toPostLargeBlock,)
      .toList();
}
